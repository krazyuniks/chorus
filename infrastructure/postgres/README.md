...
`OutboxStore` claims due rows with `FOR UPDATE SKIP LOCKED`, marks them `publishing`, increments `attempts`, and transitions rows to `sent` or `failed`. Failed rows retain `last_error` and retry through `next_attempt_at`; abandoned `publishing` leases can be released back to the retry path.
