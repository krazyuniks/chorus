"""UC3 eval playback through the real workflow and runtime activities."""
...
from chorus.workflows.types import Uc3WorkflowResult
from chorus.workflows.uc3 import UC3_WORKFLOW_TYPE, Uc3IfaSuitabilityIntakeWorkflow
from chorus.workflows.uc3_synthetic_intake import workflow_start_request_from_email_fixture

_REPO_ROOT = Path(__file__).resolve().parents[2]
_SUPPORTED_SCENARIOS = {
    "synthetic_suitability_conduct",
    "vulnerability_support_handoff",
}


@dataclass(frozen=True)
class Uc3WorkflowPlaybackResult:
    workflow_result: Uc3WorkflowResult
    captured_run: CapturedRun


def play_uc3_workflow_fixture(
    fixture: EvalFixture,
    *,
    database_url: str | None = None,
    playback_ref: str | None = None,
) -> CapturedRun:
    """Synchronously play a UC3 fixture through the real workflow path."""

    return asyncio.run(
        play_uc3_workflow_fixture_async(
            fixture,
...
async def play_uc3_workflow_fixture_async(
    fixture: EvalFixture,
    *,
    database_url: str | None = None,
    playback_ref: str | None = None,
) -> Uc3WorkflowPlaybackResult:
    """Run a UC3 eval fixture through Temporal, Agent Runtime, and Tool Gateway."""
    ...
    intake = replace(
        ...
        advice_enquiry_ref=f"{request.intake.advice_enquiry_ref}_{run_ref}",
        idempotency_key_ref=(
    ...
    with (
        ...
        async with (
            ...
            Worker(
                env.client,
                task_queue=f"eval-uc3-{fixture.fixture_id}",
        ...
        return Uc3WorkflowPlaybackResult(workflow_result=result, captured_run=captured)
...
def _validate_fixture(fixture: EvalFixture) -> None:
    ...
    if workflow_type != UC3_WORKFLOW_TYPE:
        raise ValueError(f"UC3 playback received unsupported workflow_type {workflow_type!r}")
    if fixture.scenario not in _SUPPORTED_SCENARIOS:
        raise ValueError(f"UC3 workflow playback does not support scenario {fixture.scenario!r}")


def _source_fixture_path(fixture: EvalFixture) -> Path:
    source_path = fixture.input.source_fixture_path
    if source_path is None:
        raise ValueError("UC3 workflow playback requires input.source_fixture_path")
...
def _database_url() -> str:
    ...
    if database_url is None or database_url.strip() == "":
        raise RuntimeError("CHORUS_DATABASE_URL is required for UC3 workflow playback")
    return database_url
