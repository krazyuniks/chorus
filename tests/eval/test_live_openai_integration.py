...
TEST_DATABASE_PREFIX = "chorus_live_openai_test"


def test_happy_path_fixtures_replay_through_live_openai_route(
    migrated_database_url: str,
) -> None:
    require_live_route_credential(LIVE_OPENAI_ROUTE_ID)
    ...
    with cast(
        ...
        for captured in captured_runs:
            ...
            for transcript in captured_transcripts_for_replay(captured):
                ...
                assert replay.record.alternate.runtime_route_id == LIVE_OPENAI_ROUTE_ID
                assert replay.record.alternate.provider_id == "openai"
                store.record_replay_run(replay.record)
                _assert_persisted_live_replay_record(
                    conn,
                    replay,
                    expected_route_id=LIVE_OPENAI_ROUTE_ID,
                    expected_provider_id="openai",
                    expected_outcome=outcome,
                )
                assert outcome in ALLOWED_LIVE_COMPARATOR_OUTCOMES, (
                    "live OpenAI comparator outcome for "
...
def _happy_path_captured_runs(database_url: str) -> tuple[CapturedRun, CapturedRun, CapturedRun]:
    ...
    uc2 = play_uc2_workflow_fixture(
        ...
        playback_ref=f"live-openai-{run_ref}",
    )
