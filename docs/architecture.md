...
loop. R5 also adds a code-level UC3 synthetic email advice intake adapter and
shared-worker registration plus recorded-replay route policies for UC3
workflow agent tasks; the UC3 documented operator command, triggered-run
projection evidence, full fixture playback, and live-provider activation
...
and recorded-replay model routes for the five workflow agent tasks, while the
UC3 documented operator command, triggered-run projection evidence, full
fixture playback, and live-provider activation remain deferred closure
