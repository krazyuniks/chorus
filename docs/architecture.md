...
surfaces without claiming full use-case runnable status yet.
...
shared-worker registration; UC3 provider route activation, documented
operator command, triggered-run projection evidence, and full fixture
playback remain explicit closure exceptions.
...
code-level synthetic email advice intake adapter and shared-worker
registration, while UC3 provider route activation, documented operator
command, triggered-run projection evidence, and full fixture playback remain
