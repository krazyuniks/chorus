...
| Connector inventory | Add `sandbox-attitude-to-risk-profiler` (a regulated assessment tool, modelled as an adapter contract), `sandbox-capacity-for-loss-tool` (a structured cash-flow tool), `sandbox-suitability-report-store`, `sandbox-platform-research` (read-only adapter that returns notional product universe data for the suitability check). Add `sandbox-customer-vulnerability-tool` (Consumer Duty vulnerability assessment helper). Reuse `sandbox-outbound-comms` for the gated suitability-statement send. |
| Approval policy | Suitability statement send is gated (the most direct customer-impact action). Attitude-to-risk classification confirmation is gated when the inferred classification disagrees with the customer's stated preference. Vulnerability-marker-triggered handoff is gated by adviser-level approval. |
...
| Projection sink | Add suitability-statement queue (drafts awaiting approval), attitude-to-risk classification projection, capacity-for-loss projection, vulnerability-marker projection. |
| Observability sink | Same shape. New per-use-case metrics (suitability-test pass rate, attitude-to-risk disagreement rate, vulnerability-handoff rate, suitability-statement approval latency). |
