...
| OpenTelemetry collector pipeline wired through to running services | in progress — compose-side pipeline (Tempo/Loki/Prometheus + collector exporters + Grafana datasources) and service-template auto-instrumentation (ADR 0010 §1, §2, §4) landed. Final wiring (each Workstream's services emitting via the template's `opentelemetry-instrument` entrypoint, audit-write code calling `chorus.observability.current_otel_ids()`) lands as B/C/D/E services arrive |
