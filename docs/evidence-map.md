...
| UC2 connector inventory is contract-declared | [`product-brief-uc2.md`](product-brief-uc2.md), [`domain-model-uc2.md`](domain-model-uc2.md), `contracts/connector/uc2/`, `contracts/connector/tool_call.schema.json`, `chorus/workflows/uc2.py` | Contract surface complete for conflict check, KYC / beneficial ownership, AML record store, and engagement-letter store arguments. The UC2 workflow definition now routes the declared tool names through `WorkflowSpine.connector_call` for structurally tested fake-activity runs. Runtime adapters, grants, persistence, projections, and eval fixtures remain later P4 work. |
