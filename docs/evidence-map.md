...
| UC3 connector inventory is contract-declared | [`product-brief-uc3.md`](product-brief-uc3.md), [`domain-model-uc3.md`](domain-model-uc3.md), `contracts/connector/uc3/`, `contracts/connector/tool_call.schema.json`, `tests/test_contracts.py` | Contract surface complete for attitude-to-risk profiling, capacity-for-loss assessment, platform research, suitability-report draft / issue, decline, and manual-review argument payloads. The contracts carry safe refs, bounded categories, policy refs, and conduct-hook refs only; raw client financial details, vulnerability narratives, platform credentials, and report prose stay behind intake or connector-owned stores. UC3 workflow runtime, connector adapters, grants, approval packages, projections, provider routes, local intake paths, and eval playback remain later P5 work. |
