...
| UC2 connector inventory is contract-declared | [`product-brief-uc2.md`](product-brief-uc2.md), [`domain-model-uc2.md`](domain-model-uc2.md), `contracts/connector/uc2/`, `contracts/connector/tool_call.schema.json` | Contract surface complete for conflict check, KYC / beneficial ownership, AML record store, and engagement-letter store arguments. Runtime adapters, grants, workflow calls, persistence, projections, and eval fixtures remain later P4 work. |
