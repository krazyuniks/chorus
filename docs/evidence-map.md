...
| UC2 connector inventory is contract-declared and adapter-registered | [`product-brief-uc2.md`](product-brief-uc2.md), [`domain-model-uc2.md`](domain-model-uc2.md), `contracts/connector/uc2/`, `contracts/connector/tool_call.schema.json`, `chorus/workflows/uc2.py`, `chorus/connectors/uc2.py`, `tests/connectors/test_uc2_connectors.py`, `tests/tool_gateway/test_gateway.py` | Contract surface complete for conflict check, KYC / beneficial ownership, AML record store, and engagement-letter store arguments. The UC2 workflow definition routes the declared tool names through `WorkflowSpine.connector_call`; the default connector registry now registers deterministic sandbox adapters for those tool names, and focused tests prove Tool Gateway validation uses the generated UC2 argument models. UC2 grants, approval/conduct invariants, projections, and eval fixtures remain later P4 work. |
...
| Claim | Artefacts | Status |
|---|---|---|
| The workflow spine is durable and replay-stable | `chorus/workflows/spine.py` (`WorkflowSpine`, `WorkflowDefinition`, `WorkflowStepDefinition`), `chorus/workflows/uc1.py` (UC1 enquiry-qualification workflow), `chorus/workflows/uc2.py` (UC2 legal-services intake and conflict-check workflow), `tests/workflows/` | UC1 runs on the spine. UC2 now has a definition-first workflow over the same primitives with focused Temporal tests and inline replay history; it is not yet registered as a local runnable intake path because UC2 adapters, grants, provider route support, projections, and eval fixtures are still pending P4 work. |
