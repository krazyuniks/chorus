...
| UC3 connector inventory is contract-declared and workflow-wired | [`product-brief-uc3.md`](product-brief-uc3.md), [`domain-model-uc3.md`](domain-model-uc3.md), `contracts/connector/uc3/`, `contracts/connector/tool_call.schema.json`, `chorus/workflows/uc3.py`, `tests/workflows/test_uc3_workflow.py`, `tests/test_contracts.py` | Contract surface complete for attitude-to-risk profiling, capacity-for-loss assessment, platform research, suitability-report draft / issue, decline, and manual-review argument payloads. The UC3 workflow definition routes those declared tool names through `WorkflowSpine.connector_call` using safe refs, bounded categories, policy refs, and conduct-hook refs only; raw client financial details, vulnerability narratives, platform credentials, and report prose stay behind intake or connector-owned stores. UC3 connector adapters, grants, approval-package behaviour, projections, provider routes, local intake paths, and eval playback remain later P5 work. |
