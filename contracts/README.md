...
- `intake/` — inbound channel adapters. `intake/uc1/` holds UC1 (UK personal-lines insurance broking) channel payloads; `intake/uc2/` holds UC2 legal-services intake payloads for email, corporate intake form, and intermediary referral; `intake/uc3/` holds UC3 independent-financial-advice payloads for web advice form, email, and introducer referral. Port-shared intake schemas sit at the port root.
...
- `connector/` — Tool Gateway connector contracts: the `tool_call` and `gateway_verdict` envelopes plus per-tool argument schemas. `connector/uc1/` holds UC1-specific tool payloads; `connector/uc2/` holds UC2 argument payloads for conflict check, KYC / beneficial ownership, AML record-store, and engagement-letter-store tools; `connector/uc3/` holds UC3 argument payloads for attitude-to-risk profiling, capacity-for-loss assessment, platform research, and suitability-report-store tools.
...
- `projection/` — domain event-stream contract (`workflow_event`) and read-model schemas. The shared workflow event contract admits the declared UC1, UC2, and UC3 workflow families and their safe root-subject refs; the current R4 surface projects shared workflow progress and generic approval-package inspection for UC2 and UC3 when rows exist.
