...
- `intake/` — inbound channel adapters. `intake/uc1/` holds UC1 (UK personal-lines insurance broking) channel payloads; `intake/uc2/` holds UC2 legal-services intake payloads for email, corporate intake form, and intermediary referral. Port-shared intake schemas sit at the port root.
...
- `connector/` — Tool Gateway connector contracts: the `tool_call` and `gateway_verdict` envelopes plus per-tool argument schemas. `connector/uc1/` holds UC1-specific tool payloads; `connector/uc2/` holds UC2 argument payloads for conflict check, KYC / beneficial ownership, AML record-store, and engagement-letter-store tools.
